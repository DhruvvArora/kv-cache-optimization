...
PROMPT_LENGTHS = [256, 512, 768, 1024]
MAX_NEW_TOKENS = 64
RUNS_PER_SETTING = 3
...
def make_prompt(tokenizer, target_tokens: int) -> str:
    ...
    # Trim exactly to target length and decode back
...
def run_experiment():
    ...
    for prompt_len in PROMPT_LENGTHS:
        ...
        for run_idx in range(RUNS_PER_SETTING):
            ...
            avg_time_per_generated_token = total_latency / max(output_tokens, 1)
            ...
            results.append({
                ...
                "avg_decode_sec_per_token": round(avg_time_per_generated_token, 4),
            ...
            print(
                ...
                f"Avg/token={avg_time_per_generated_token:.4f}s, "
                f"Peak VRAM={peak_vram_mb:.2f} MB"
            )

    df = pd.DataFrame(results)
    df.to_csv("kv_baseline_results.csv", index=False)
    print("\nSaved results to kv_baseline_results.csv")
    ...
    summary.to_csv("kv_baseline_summary.csv", index=False)
    print("Saved summary to kv_baseline_summary.csv")
    print("\nSummary:")
    print(summary)
