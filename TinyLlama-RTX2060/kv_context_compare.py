...
def run_experiment():
    ...
    for prompt_len in PROMPT_LENGTHS:
        ...
        for run_idx in range(RUNS_PER_SETTING):
            ...
            results.append({
                "mode": "full_context" if prompt_len == 1024 else "reduced_context",
            ...
            print(
                f"Mode={'full_context' if prompt_len == 1024 else 'reduced_context'}, "
    ...
    df.to_csv("kv_context_compare_results.csv", index=False)
    print("\nSaved results to kv_context_compare_results.csv")

    summary = df.groupby(["mode", "prompt_tokens"], as_index=False).agg({
        "generated_tokens": "mean",
        "total_latency_sec": "mean",
        "avg_decode_sec_per_token": "mean",
        "peak_vram_mb": "mean"
    })
    summary.to_csv("kv_context_compare_summary.csv", index=False)
    print("Saved summary to kv_context_compare_summary.csv")
    print("\nSummary:")
    print(summary)
